    ...
    for data in collector:
        ...
        if experience_replay_buffer:
            ...
            terminated_smiles = next_data.get_sub_tensordict(idx=terminated).select(
                "SMILES", "SMILES2", "reward", "step_count")
            experience_replay_buffer.extend(terminated_smiles)
            ...
            for i in range(num_mini_batches):
                ...
                if experience_replay_buffer:
                    ...
                    cat_replay_data, split_replay_batch = create_batch_from_replay_smiles(
                        replay_batch, device, reward_key="reward")
                    with torch.no_grad():
                        replay_batch = actor_training(replay_batch)
                        replay_batch = adv_module(replay_batch.exclude(
                            ("next", "recurrent_state_c"),
                            ("next", "recurrent_state_h"),
                        ))
                    replay_loss = loss_module(replay_batch)
                    replay_loss_sum = replay_loss["loss_critic"] + replay_loss["loss_objective"] + \
                        replay_loss["loss_entropy"]
                    ...
                    replay_losses[j, i] = TensorDict({
                        ...
                        "replay_reward": cat_replay_data["next"]["reward"][cat_replay_data["next"]["done"]].mean().item(),
                    }, batch_size=[])
                    replay_loss_sum.backward()
