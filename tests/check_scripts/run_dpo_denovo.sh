...
#SBATCH --job-name=dpo_denovo
#SBATCH --ntasks=6
#SBATCH --cpus-per-task=1
#SBATCH --gres=gpu:1
#SBATCH --output=slurm_logs/dpo_denovo%j.txt
#SBATCH --error=slurm_errors/dpo_denovo%j.txt

current_commit=$(git rev-parse --short HEAD)
project_name="acegen-scripts-check-$current_commit"
agent_name="dpo_denovo"
if [ -z "$N_RUN" ]; then
...
python $PYTHONPATH/scripts/dpo/dpo.py \
