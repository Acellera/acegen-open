...
#SBATCH --job-name=augmem_denovo
#SBATCH --ntasks=6
#SBATCH --cpus-per-task=1
#SBATCH --gres=gpu:1
#SBATCH --output=slurm_logs/augmem_denovo%j.txt
#SBATCH --error=slurm_errors/augmem_denovo%j.txt

current_commit=$(git rev-parse --short HEAD)
project_name="acegen-scripts-check-$current_commit"
agent_name="augmem_denovo"
if [ -z "$N_RUN" ]; then
...
python $PYTHONPATH/scripts/augmented_memory/augmem.py \
