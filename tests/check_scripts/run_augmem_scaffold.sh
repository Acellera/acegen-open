...
#SBATCH --job-name=augmem_scaffold
#SBATCH --ntasks=6
#SBATCH --cpus-per-task=1
#SBATCH --gres=gpu:1
#SBATCH --output=slurm_logs/augmem_scaffold%j.txt
#SBATCH --error=slurm_errors/augmem_scaffold%j.txt

current_commit=$(git rev-parse --short HEAD)
project_name="acegen-scripts-check-$current_commit"
agent_name="augmem_scaffold"
if [ -z "$N_RUN" ]; then
