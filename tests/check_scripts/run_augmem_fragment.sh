...
#SBATCH --job-name=augmem_fragment
#SBATCH --ntasks=6
#SBATCH --cpus-per-task=1
#SBATCH --gres=gpu:1
#SBATCH --output=slurm_logs/augmem_fragment%j.txt
#SBATCH --error=slurm_errors/augmem_fragment%j.txt

current_commit=$(git rev-parse --short HEAD)
project_name="acegen-scripts-check-$current_commit"
agent_name="augmem_fragment"
if [ -z "$N_RUN" ]; then
...
python $PYTHONPATH/scripts/augmented_memory/augmem.py --config-name config_fragment \
