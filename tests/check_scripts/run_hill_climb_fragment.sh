...
#SBATCH --job-name=hill_climb_fragment
#SBATCH --ntasks=6
#SBATCH --cpus-per-task=1
#SBATCH --gres=gpu:1
#SBATCH --output=slurm_logs/hill_climb_fragment%j.txt
#SBATCH --error=slurm_errors/hill_climb_fragment%j.txt

current_commit=$(git rev-parse --short HEAD)
project_name="acegen-scripts-check-$current_commit"
agent_name="hill_climb_fragment"
if [ -z "$N_RUN" ]; then
...
python $PYTHONPATH/scripts/hill_climb/hill_climb.py --config-name config_fragment \
