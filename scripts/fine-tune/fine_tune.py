...
def main(cfg: "DictConfig"):
    ...
    # Check the coverage of the vocabulary on the dataset
    vocabulary_coverage = (dataset._n_input / len(dataset)) * 100
    n_failed = len(dataset) - dataset._n_input
    if n_failed > 0:
        logging.warning(
            f"{n_failed} molecules failed vocabulary encoding ({vocabulary_coverage:.1%})"
            f"Proceeding with remaining dataset"
        )
    ...
    # Save Prior
    save_path = Path(cfg.model_log_dir) / f"prior_actor.pt"
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        save_path = Path(cfg.model_log_dir) / f"finetuned_actor_epoch_{epoch}.pt"
        torch.save(actor_training.state_dict(), save_path)
