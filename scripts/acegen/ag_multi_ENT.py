...
def run_reinforce(cfg, task):
    ...
    while not task.finished:
        ...
        # ---- Population loss -----
        if cfg.get("pop_entropy_coef", False):
            for i, actor_training, loss in zip(
                ...
                if pop_pps:
                    ...
                    prob_dist = torch.distributions.Categorical(logits=-pop_pps)
                    entropy = prob_dist.entropy()
                    pop_entropy_loss = cfg.pop_entropy_coef * entropy.mean()
                    log_info[f"agent{i}/entropy"] = entropy.mean().item()
                    log_info[f"agent{i}/entropy_loss"] = pop_entropy_loss.item()
                    loss += pop_entropy_loss
