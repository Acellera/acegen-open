...
def main(cfg: "DictConfig"):
    run_taks(cfg, run_reinforce, __file__)
...
def run_reinforce(cfg, task):
    ...
    while not task.finished:
        ...
        for i, actor_inference, actor_training, experience_replay_buffer, optim, scheduler, baseline in zip(
            ...
            log_info.update({f"agent{i}/RND_loss": RND_loss.item()})
...
def compute_loss(
    data, model, prior, alpha=1, sigma=0.0, baseline=None, entropy_coef=0.0, rnd_target=None, rnd_pred=None, rnd_baseline=None
):
