        ...
        for i, actor_inference, actor_training, experience_replay_buffer, optim, scheduler, baseline in zip(
            ...
            if cfg.get("pop_diff_coef", False):
                ...
                for actor in population_training[:i]:
                    ...
                    with torch.no_grad():
                        agent_j_likelihood, _ = get_log_prob(data, actor)
                    # Loss 
                    scaled_diff = reward * torch.abs(agent_likelihood - agent_j_likelihood)
                    pop_differences.append(scaled_diff)
                    actor = actor.cpu()
                
                if pop_differences:
                    pop_diff = torch.stack(
                        pop_differences, dim=1
                    ).sum(dim=1)
                    pop_diff_loss = cfg.pop_diff_coef * pop_diff
                    log_info[f"agent{i}/diff"] = pop_diff.mean().item()
                    log_info[f"agent{i}/diff_loss"] = pop_diff_loss.mean().item()
                    loss -= pop_diff_loss
