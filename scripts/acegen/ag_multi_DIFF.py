...
def run_reinforce(cfg, task):
    ...
    while not task.finished:
        ...
        for i, actor_inference, actor_training, experience_replay_buffer, optim, scheduler, baseline in zip(
            ...
            # Compute loss
            data, loss, agent_likelihood, agent_perplexity = compute_loss(
                data,
                actor_training,
                prior,
        ...
        # ---- Population loss -----
        if cfg.get("pop_diff_coef", False):
            for i, actor_training, loss in zip(
                ...
                if pop_pps:
                    diff = torch.abs(torch.stack(pop_pps, dim=1).mean() - agent_perplexity.detach().mean())
                    pop_diff_loss = cfg.pop_diff_coef * diff
                    log_info[f"agent{i}/diff"] = diff.item()
                    log_info[f"agent{i}/diff_loss"] = pop_diff_loss.item()
                    loss -= pop_diff_loss
...
def compute_loss(
    ...
    agent_likelihood, agent_perplexity, agent_dist = get_log_prob(data, model)
    ...
    return data, loss, agent_likelihood, agent_perplexity
