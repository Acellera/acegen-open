...
def rbf_kernel(X, Y=None, gamma=None):
    ...
    gamma : float, optional
        Kernel coefficient. If None, defaults to 1.0 / (n_features * X.var()).

    Returns
    -------
    K : torch.Tensor of shape (n_samples_X, n_samples_Y)
    ...
    dist_sq = torch.clamp(dist_sq, min=0.0)  # Ensure all distances are non-negative
...
def run_reinforce(cfg, task):
    ...
    while not task.finished:
        ...
        # ---- Population loss -----
        if cfg.get("DvD_size", False):
            # Randomly select non terminated states
            all_seqs = []
            for data in population_data:
                all_seqs.append(data["action"].detach())
            all_seqs = torch.cat(all_seqs, dim=0)
            sample_idxs = torch.randint(0, len(all_seqs), (cfg.DvD_size,))
            sample_states = torch.cat([torch.randint(0, torch.where(all_seqs[sidx] == 0)[0][0], (1,)) for sidx in sample_idxs], dim=0)
            sample_seqs = torch.vstack([all_seqs[i] for i in sample_idxs])
            sample_subsets = [smiles_to_tensordict(sample_seqs, mask_value=0, device=device) for _ in range(num_agents)]

            # Get behaviour policy per agent, for sampled states
            behaviour_embeddings = []
            for actor_training, sample_seqs in zip(population_training, sample_subsets):
                embedding = []
                actor_training = actor_training.to(device)
                with torch.no_grad():
                    agent_data = actor_training(sample_seqs)
                    log_prob, dist = get_log_prob(agent_data, actor_training)
                    probs = dist.probs
                    for sample_idx, state_idx in enumerate(sample_states):
                        embedding.append(probs[sample_idx, state_idx])
                    behaviour_embeddings.append(torch.cat(embedding, dim=0))
                actor_training = actor_training.cpu()
            behaviour_embeddings = torch.vstack(behaviour_embeddings)
            
            # Normalize with rbf kernel and get determinant
            behaviour_embeddings = rbf_kernel(behaviour_embeddings)
            DvD_det = torch.linalg.det(behaviour_embeddings)
