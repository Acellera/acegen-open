...
def run_reinforce(cfg, task):
    ...
    population_loss_f = torch.nn.CrossEntropyLoss()
    ...
    while not task.finished:
        ...
        # ---- Population loss -----
        if cfg.get("pop_cross_entropy_coef", False):
            for i, actor_training, loss in zip(
                range(num_agents),
                population_training,
                population_loss,
            ):
                ...
                pop_pps = []
                # Iterative over previous agents
                for j in range(i+1):
                    data = population_data[j].to(device)
                    _, perplexity, _ = get_log_prob(data, actor_training)
                    pop_pps.append(perplexity)
                    data = data.cpu()

                if pop_pps:
                    # NOTE batch grouping is arbitrary and state perplexities is a proxy for behavior
                    # therefore, we sum the perplexities over the batch
                    pop_pps = torch.stack(pop_pps, dim=1).sum(dim=0)
                    target = torch.zeros_like(pop_pps, dtype=torch.float)
                    target[-1] = 1.0
                    # NOTE we reverse perplexities to act as logits where higher is more likely 
                    pop_ce = population_loss_f(-pop_pps, target)
                    pop_ce_loss = cfg.pop_cross_entropy_coef * pop_ce.mean()
                    log_info[f"agent{i}/cross_entropy"] = pop_ce.mean().item()
                    log_info[f"agent{i}/cross_entropy_loss"] = pop_ce_loss.item()
                    loss += pop_ce_loss
...
def get_log_prob(data, model):
    ...
    perplexity = torch.exp(-log_prob / mask.sum(-1))
    return log_prob, perplexity, dist
...
def compute_loss(
    ...
    with torch.no_grad():
        prior_likelihood, _, prior_dist = get_log_prob(data, prior)

    agent_likelihood, _, agent_dist = get_log_prob(data, model)
