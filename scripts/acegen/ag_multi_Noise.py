...
def run_reinforce(cfg, task):
    ...
    for _ in range(num_agents):
        params = deepcopy(ckpt)
        # Add noise
        if cfg.get("pop_init_noise", False):
            for k in params.keys():
                if ('weight' in k) and ('embedding' not in k): # Or just params['module.0.module.2.module.0.weight'] for final linear layer
                    layer_weights = params[k]
                    noise = (torch.randn(layer_weights.size()) * cfg.pop_init_noise).to(device)
                    layer_weights.add_(noise)
        actor_training, actor_inference = create_actor(vocabulary_size=len(vocabulary))
        actor_inference.load_state_dict(
            adapt_state_dict(params, actor_inference.state_dict())
        )
